...
######################################################################
# Deploy the Curity Identity Server to integrate with SPIRE X509 SVIDs
######################################################################
...
#
# Configure the cert-manmager root CA as a client trust store
#
...
#
kubectl -n curity apply -f idsvr/client-certificate-forwarder.yaml
if [ $? -ne 0 ]; then
  exit 1
fi
